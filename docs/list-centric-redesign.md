...
**Status: mostly implemented, on the `list-centric-redesign` branch, not yet merged to `main`.** Every route/list kind described below is real and verified against live data, including real per-group table rendering for the `group-by-membership` combine mode (one real table per group, most-sources-first, verified live against a hand-built 3-source combine). What's left before this can merge: the combine setup dialog to actually *create* a dynamic list from the UI at all (`HomeRoute.tsx`'s "+ New list" only creates manual lists today — a `group-by-membership` list, and dynamic lists generally, are only reachable right now by hand-editing the stored `lists` array, which is how the rendering above was verified) — until that exists, nobody can actually rebuild the old Comparison page's multi-account experience through the UI, so `public/app.tsx` stays. See `CLAUDE.md`'s own top-of-file note for the exact current state. Once the combine dialog lands and this merges, fold this doc's content into `CLAUDE.md`'s own Architecture section (and delete or shrink this doc) rather than maintaining both as separate sources of truth long-term.
...
- **The combine setup dialog** — creating a *dynamic* list from the UI at all. `HomeRoute.tsx`'s "+ New list" only ever creates a manual (empty, hand-populated) list today; a dynamic list (including `group-by-membership`, whose rendering side is otherwise done) is only reachable by hand-editing the stored `lists` array. This is the one piece standing between the new app and deleting `app.tsx`'s old multi-slot comparison page for good — the rendering is proven, but nobody can actually *build* a comparison through the UI yet.
