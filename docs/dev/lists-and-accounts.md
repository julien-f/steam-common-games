...
**Every pref key carries its own `updatedAt`** (`prefs.ts`'s internal storage format — `getPref`/`setPref`'s own signatures are unchanged, callers never see the timestamp). Once signed in, `setPref` pushes the changed key's `{ value, updatedAt }` to `PUT /api/me/prefs/:key` — per key, never a whole-blob PUT, so two keys changing around the same time on different tabs/devices can't clobber each other. The server applies the same rule on its own side (`setUserPref`, `lib/auth.js`): a write older than what's already stored is rejected rather than clobbering a newer one.

**Every sign-in check merges prefs with the server, not just the first one** (`authStore.ts`'s `syncPrefsWithServer`, run from `initAuth` on every page load), by per-key last-write-wins — but a device's *first ever* sync with a given account is treated differently from every sync after it:
