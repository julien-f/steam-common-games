...
- **Focus** — `nextPair`/`answer`/`progress` take an optional `focus` set (the list page's "Compare N selected"): only its unranked games become candidates, opponents still come from the whole ranking. Passed to `/lists/:listId/rank` as router history state (`rankFocus`), not in the URL — it survives a reload and ends when the screen is left or "Compare all instead" replaces the state. A mid-way insertion of a game outside the focus is dropped and restarts later.
