...
Accounts, lists, folders and preferences are `localStorage` only (`prefs.ts`), optionally synced to the server once signed in with Steam. Signing in (the account chip's popover — `AccountChip.tsx`) is entirely optional; without it the app behaves exactly as before. Once signed in, `prefs.ts`'s `setPref` additionally pushes each changed key to `PUT /api/me/prefs/:key` (`authStore.ts` sets the "currently signed in" flag `setPref` checks) — per key, never a whole-blob PUT, so two keys changing around the same time on different tabs/devices can't clobber each other.

**First sign-in on a given browser** (`authStore.ts`'s `reconcilePrefsOnFirstLogin`, gated by a one-time `localStorage` flag per steamid so it never re-runs): if the account has no server-side prefs yet, this browser's `localStorage` is pushed up wholesale; if this browser has nothing locally, the account's server-side prefs are adopted (and the page reloads, since every store seeds its in-memory state from `localStorage` once at load — simpler than live-patching each of them); if both sides already have data, `DataChoiceModal.tsx` asks the user which to keep, since nothing here can guess whether the browser's data or the account's is the one worth keeping. See [architecture.md](architecture.md) for the underlying routes.
...
Pref keys: `schemaVersion`, `myAccount`, `currentAccount`, `recentAccounts`, `lists`, `folders`, `recentGames` (backs the `recent-games` system list), plus the shared table-view keys for the fixed system kinds (`ownedListView`, `wishlistListView`, `bundleListView` and `compareListView` — each shared across all bundles/comparisons, unlike user lists which each keep their own `tableView` — and `recentListView`), and `bundlesBrowseView` for `/bundles`' own bundle-picker table (a table of bundles, not of games).
