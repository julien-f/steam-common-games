...
//   cached long-term (`tagnames:all`, in the same 60-day meta TTL group as `tags:`) rather
...
async function getSteamTags(appid, { force = false } = {}) {
  ...
  // A `tags:` entry written before this function switched from SteamSpy's
  // `{tagname: voteCount}` object to a `tagid[]` array is still sitting in the 60-day
  // cache_meta table under the old shape — treat it as a miss rather than crash on it
  // (`tagIds.slice`), so the stale entry is quietly replaced by a fresh fetch instead of
  // requiring a full cache wipe (a schema version bump) just for this one key's format.
  const validCachedIds = Array.isArray(cachedIds) ? cachedIds : undefined;

  const [tagIds, nameMap] = await Promise.all([
    validCachedIds !== undefined
      ? validCachedIds
