...
//
// - `IStoreBrowseService/GetItems` (api.steampowered.com) returns each app's `tagids`,
//   already ordered by relevance weight descending — same order the store page displays
//   them in. One call per game, same as SteamSpy needed. Requested (and shown) up to 30 —
//   the store page itself shows "up to 20 popular tags" but a game's full tag list can run
//   a bit past that; there's no further display cap of our own on top of this.
...
//   cached long-term (`tagnames:all`, in the same 60-day meta TTL group as `tagids:`) rather
...
async function fetchTagIds(appid) {
  ...
  return (item?.success === 1 && Array.isArray(item.tagids)) ? item.tagids : [];
}

async function getSteamTags(appid, { force = false } = {}) {
  // Deliberately a new key prefix (`tagids:`), not a reuse of the old SteamSpy-era `tags:`
  // key: that one stored a `{tagname: voteCount}` object, but for a game with no SteamSpy
  // votes at all it stored a bare `[]` — indistinguishable, by shape alone, from a
  // legitimate empty `tagid[]` result under this new format. A shape check (`Array.isArray`)
  // can't tell those apart, so it would leave every "no SteamSpy tags" game permanently
  // stuck showing no tags even after Steam's own data (fetched below) has some. A fresh key
  // sidesteps the ambiguity entirely — old `tags:` entries are simply never read again and
  // age out on their own after the existing 60-day meta TTL.
  const cacheKey = `tagids:${appid}`;
  const cachedIds = getCached(cacheKey, { force });

  const [tagIds, nameMap] = await Promise.all([
    cachedIds !== undefined
      ? cachedIds
      : withDedup(cacheKey, async () => {
          const ids = await fetchTagIds(appid);
  ...
  return tagIds.map(id => nameMap[id]).filter(Boolean);
}
...
module.exports = { resolveSteamId, getOwnedGames, getWishlist, getPlayerSummaries, getGameRating, getAppDetails, getSteamTags, searchStoreGames, getProtonDbStatus, getGameSchema, getPlayerAchievements, getGlobalAchievementPercentages, getGameNews };
