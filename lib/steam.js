...
async function getPlayerSummaries(steamIds, { force = false } = {}) {
  const result = new Map();
  const uncached = [];
  for (const id of steamIds) {
    const hit = getCached(`player:${id}`, { force });
...
function extractAppDetails(raw, appid) {
  ...
  const {
    ...
    capsule_imagev5 = null, capsule_image = null,
  } = raw;
  return {
    name,
...
module.exports = { resolveSteamId, getOwnedGames, getWishlist, getPlayerSummaries, getGameRating, getAppDetails, getSteamSpyTags };
