...
// Coalesces concurrent getAppDetails calls into batched ?appids= requests.
// All calls arriving within getDelay() ms are grouped into one fetch; batches
// are capped at maxBatch to stay within Steam's undocumented batch limit.
function createBatchCoalescer(fetchBatch, { maxBatch = 25, getDelay = () => 20 } = {}) {
  let pending = new Map(); // appid -> [{resolve, reject}]
  let timer = null;
  ...
  function flush() {
    ...
    fetchBatch(ids).then(
      results => {
        for (const [id, waiters] of current) {
  ...
  return function enqueue(id) {
    return new Promise((resolve, reject) => {
      ...
      if (pending.size >= maxBatch) { clearTimeout(timer); flush(); }
      else if (!timer) timer = setTimeout(flush, getDelay());
    });
  };
}
...
async function fetchAppDetailsBatch(appids) {
  const res = await fetchStoreApi(
    `https://store.steampowered.com/api/appdetails?appids=${appids.join(',')}&l=english`
  );
  ...
  for (const appid of appids) {
    ...
    if (!entry?.success || !entry.data) { results.set(appid, null); continue; }
...
let _coalescerDelay = 20;
const batchAppDetails = createBatchCoalescer(fetchAppDetailsBatch, { getDelay: () => _coalescerDelay });

function getAppDetails(appid) {
  return batchAppDetails(appid);
}

module.exports = {
  resolveSteamId, getOwnedGames, getPlayerSummaries, getGameRating, getAppDetails,
  _setCoalescerDelay: ms => { _coalescerDelay = ms; },
};
