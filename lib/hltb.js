...
function pickBestMatch(data, name) {
  ...
  const best = data.reduce((best, e) => {
    ...
    return sim > best.sim
      ? { id: e.game_id, main: Math.round(e.comp_main / 3600), extra: Math.round(e.comp_plus / 3600), completionist: e.comp_100 ? Math.round(e.comp_100 / 3600) : null, sim }
      : best;
  }, { sim: -1 });
  if (best.sim < 0.35) return null;

  return { id: best.id, main: best.main, extra: best.extra, completionist: best.completionist ?? null };
}
