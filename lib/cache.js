...
const { CACHE_TTL_MS, RESOLVE_CACHE_TTL_MS, RATING_CACHE_TTL_MS, META_CACHE_TTL_MS } = require('./config');

const CACHE_FILE = 'CACHE_FILE' in process.env
  ? process.env.CACHE_FILE
  : path.join(__dirname, '..', 'cache.json');
const CACHE_FILE_TMP = CACHE_FILE && CACHE_FILE + '.tmp';

// Increment when the cached value format changes (raw vs transformed).
// Existing cache.json files with a different __v are discarded on startup.
const CACHE_SCHEMA_VERSION = 2;

function getTtlForKey(key) {
  if (key.startsWith('resolve:')) return RESOLVE_CACHE_TTL_MS;
  if (key.startsWith('rating:'))  return RATING_CACHE_TTL_MS;
  if (key.startsWith('hltb:') || key.startsWith('meta:') || key.startsWith('tags:')) return META_CACHE_TTL_MS;
  return CACHE_TTL_MS;
}

const cache = (() => {
  if (!CACHE_FILE) return new Map();
  for (const file of [CACHE_FILE, CACHE_FILE_TMP]) {
    try {
      ...
      if (data.__v !== CACHE_SCHEMA_VERSION) {
        console.log('[cache] Schema version mismatch — cache cleared');
        return new Map();
      }
      const entries = Object.entries(data).filter(([k]) => k !== '__v');
...
function saveCacheToDisk() {
  ...
  evictExpired();
  try {
    const obj = { __v: CACHE_SCHEMA_VERSION };
    for (const [k, v] of cache) obj[k] = v;
    fs.writeFileSync(CACHE_FILE_TMP, JSON.stringify(obj));
