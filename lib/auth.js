...
const stmts = {
  upsertUser: db.prepare(`
    INSERT INTO users (steamid, prefs, created_at, updated_at) VALUES (?, '{}', ?, ?)
    ON CONFLICT(steamid) DO UPDATE SET updated_at = excluded.updated_at
  `),
  getUser: db.prepare('SELECT steamid, prefs FROM users WHERE steamid = ?'),
  setPrefs: db.prepare('UPDATE users SET prefs = ?, updated_at = ? WHERE steamid = ?'),
...
function getSessionUser(sessionId) {
  ...
  const user = stmts.getUser.get(session.steamid);
  return user && { steamid: user.steamid, prefs: JSON.parse(user.prefs) };
}

// Merges one key into the user's prefs blob — never a whole-blob overwrite, mirroring prefs.ts's
// own per-key setPref (see its comment): two keys changing around the same time, from different
// tabs or devices, must not be able to clobber each other. The read-modify-write below is safe
// against that race within this single process only, same caveat lib/dedup.js's in-flight map
// already carries — DatabaseSync's calls are synchronous, so nothing can interleave between the
// read and the write in the same turn of the event loop.
function setUserPref(steamid, key, value) {
  const user = stmts.getUser.get(steamid);
  const prefs = user ? JSON.parse(user.prefs) : {};
  prefs[key] = value;
  stmts.setPrefs.run(JSON.stringify(prefs), Date.now(), steamid);
}
