...
const stmts = {
  ...
  getUserPref: db.prepare('SELECT updated_at FROM user_prefs WHERE steamid = ? AND key = ?'),
...
// Sets one key, but only if `updatedAt` is at least as new as what's already stored — the
// server-side half of the same last-write-wins rule the frontend merge applies, so two devices
// racing to save the same key (network reordering, one briefly offline) converge on whichever
// edit actually happened last instead of whichever request happened to arrive last. Trusts the
// caller's clock for `updatedAt` (Date.now() on whichever device made the edit) rather than
// this server's receipt time — acceptable for this app's scale (a handful of a person's own
// devices), not a general-purpose distributed clock.
function setUserPref(steamid, key, value, updatedAt) {
  const existing = stmts.getUserPref.get(steamid, key);
