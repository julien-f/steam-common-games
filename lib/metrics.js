...
function createNamedCounter() {
  ...
  function summarize() {
    ...
    return Object.fromEntries(Object.keys(counters).map(name =>
      [name, { sinceRestart: counters[name].count, lastHour: counters[name].recent.filter(e => e.ts >= cutoff).length }]
    ));
  }
  function reset() {
    for (const key of Object.keys(counters)) delete counters[key];
  }
  return { record, summarize, reset };
}
...
function getMetrics() {
  return {
    ...
    rateLimiters: limiterCounter.summarize(),
    dedupHits: dedupCounter.summarize(),
    cacheHits: Object.fromEntries(Object.keys(cacheEvents).map(group => {
      const c = cacheEvents[group];
      const cutoff = Date.now() - WINDOW_MS;
      const lastHour = { hits: 0, misses: 0, forced: 0 };
      for (const e of c.recent) if (e.ts >= cutoff) lastHour[e.outcome]++;
      return [group, { sinceRestart: { hits: c.hits, misses: c.misses, forced: c.forced }, lastHour }];
    })),
  };
}
